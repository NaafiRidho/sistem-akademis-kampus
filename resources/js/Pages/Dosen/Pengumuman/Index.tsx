...
export default function Index({ pengumuman, unreadCount, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('dosen.pengumuman.index'), { 
            search, 
    ...
    return (
        <div className="flex h-screen bg-gray-100">
            <DosenSidebar sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
            <div className="flex-1 flex flex-col overflow-hidden">
                <Header 
                    ...
                    title="Pengumuman"
                    subtitle="Pengumuman untuk dosen"
                />
                            ...
                            <div className="space-y-4">
                                {pengumuman.data.length === 0 ? (
                                    ...
                                    pengumuman.data.map((item) => (
                                        <Link
                                            key={item.id}
                                            href={route('dosen.pengumuman.show', item.id)}
                                            className={`block border rounded-lg p-4 hover:shadow-md transition ${
                                                !item.isRead ? 'bg-blue-50 border-blue-300' : 'bg-white border-gray-200'
                                            }`}
                                        >
