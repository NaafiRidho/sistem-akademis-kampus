        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                try {
                    ...
                    $alreadyExists = Nilai::where('mahasiswa_id', $mahasiswa->id)
                        ->where('mata_kuliah_id', $mataKuliah->id)
                        ->where('semester', $row[2])
                        ->where('tahun_ajaran', $row[3])
                        ->exists();

                    if ($alreadyExists) {
                        $errors[] = "Baris " . ($index + 1) . ": Nilai untuk NIM '{$row[0]}' pada mata kuliah ini sudah ada";
                        continue;
                    }
