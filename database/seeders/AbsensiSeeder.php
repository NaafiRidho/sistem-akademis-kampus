        ...
        foreach ($jadwal as $jadwalItem) {
            ...
            for ($pertemuan = 1; $pertemuan <= 8; $pertemuan++) {
                ...
                foreach ($mahasiswa as $mhs) {
                    ...
                    $keterangan = null;
                    if ($status === 'Sakit') {
                        $keteranganSakit = ['Demam', 'Flu', 'Sakit kepala', 'Sakit perut', 'Batuk pilek'];
                        $keterangan = $keteranganSakit[array_rand($keteranganSakit)];
                    } elseif ($status === 'Izin') {
                        $keteranganIzin = ['Keperluan keluarga', 'Acara penting', 'Urusan pribadi', 'Acara keluarga'];
                        $keterangan = $keteranganIzin[array_rand($keteranganIzin)];
                    } elseif ($status === 'Alpa') {
                        if (rand(0, 1)) {
                            $keteranganAlpa = ['Terlambat bangun', 'Lupa jadwal', 'Kendaraan rusak'];
                            $keterangan = $keteranganAlpa[array_rand($keteranganAlpa)];
                        }
                    }
